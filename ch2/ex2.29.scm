...
(define (total-weight mobile)
  (define (recur structure)
    (cond ((null? structure) 0)
          (not (pair? structure) structure)
          (else (total-weight structure))))
  (+ (recur (branch-structure (left-branch mobile)))
     (recur (branch-structure (right-branch mobile)))))
...
;;; d. {{{
;;;
;;; Suppose we change the representation of mobiles so that the constructors
;;; are
...
;; Only the functions in part (a) need to be redefined, since all other access
;; to the components of a mobile or branch are through accessor functions:
