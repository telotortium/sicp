...
The ~cond~ with recipient is a little more complex:

#+BEGIN_SRC scheme
  (cond (a => proc) (else b)
        ;; Equivalent to (if a (proc a) b)
        ;; Equivalent to ((lambda (x y) (if x (proc x) y)) a b)
        )
#+END_SRC

Unfortunately, I don't have a gensym, so perhaps I need to introduce the
environment here.
        ...
        (let ((first (car clauses))
          ...
          (cond ((cond-else-clause? first)
                ...
                ((cond-recipient-clause? first)
                 (let ((test-res (eval-4-1 ...)))
                   (make-if (cond-predicate))))
