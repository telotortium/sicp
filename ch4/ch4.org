...
#+BEGIN_SRC scheme :tangle interp-sec-4-1
  ...
  #+BEGIN_SRC scheme
    ...
    (define (eval-4-1 exp env)
      (cond ((self-evaluating? exp) exp)
            ...
            ((application? exp)
             (apply (eval-4-1 (operator exp) env)
...
**** NEXT [[http://mitpress.mit.edu/sicp/full-text/book/book-Z-H-26.html#%_thm_4.3][Exercise 4.3]]
     ...
     #+BEGIN_SRC scheme :tangle symbol-table.scm
       (define (put table op type proc)
         (hash-table-set! table (list op type) proc))
       (define (get table op type)
         (hash-table-ref/default table (list op type) '()))
     #+END_SRC
...
* Tasks
** TODO Important Programming Concepts (Even on Embedded Systems)
   [2015-01-10 Sat]
** TODO Important Programming Concepts (Even on Embedded Systems)
   [2015-01-10 Sat]
